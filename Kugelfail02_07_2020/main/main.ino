...
void loop() {
  ...
  if (routine.goLittleBall(discmonitor.getSpeed())){
		Serial.print("> Erlaubnis erteilt!");
    ...
    if (discmonitor.currentPosition != lastCheckPosition) {
      ...
      if (triggerValid && triggerPosition == nextPos) { //Heißt: Wir erwarten noch eine Flanke, bis der Timer läuft und dann ausgelöst wird
        triggerValid = true;
        Serial.print("> Ausloesen vorbereitet! (seg ");     
        Serial.print(triggerPosition);   
        Serial.print(", time");   
        Serial.print(triggerDelay);   
        Serial.print(", rel pos in seg ");   
        Serial.print((float)triggerDelay / (float)discmonitor.segmentTimes[triggerPosition], 3);   
        Serial.println(")");   
      }
      else {
